...
base_pkgs <- c("data.table", "ggplot2", "patchwork", "rmarkdown", "knitr", "jsonlite", "htmltools")
...
error_style <- "shading" 
if (interactive()) {
  message("\nHow should the 95% error analysis be displayed on the plots?")
  message("  [1] Shading (95% Ribbon)")
  message("  [2] Error Bars (centered on points)")
  ans_err <- trimws(readline("Choose 1 or 2 [default 1]: "))
  if (ans_err == "2") error_style <- "bars"
}
# -----------------------------
set.seed(seed)
...
compute_two_panel <- function(dt, source_label, output_dir, file_tag,
  ...
  # ---- Panel A ----
  # ---- Panel A (Updated for 95% CI Inference) ----
  rep_means <- dt_use[, .(rep_mean = mean(Abundance, na.rm = TRUE)), by = .(Day, SampleID)]
  
  A_df <- rep_means[, .(
    mean_val = mean(rep_mean, na.rm = TRUE),
    sd_val   = sd(rep_mean, na.rm = TRUE),
    n_rep    = .N
  ), by = Day][order(Day)]
  
  # Calculate 95% Confidence Interval (Inferential Error Analysis)
  A_df[, sem := sd_val / sqrt(n_rep)]
  A_df[, band_lo := mean_val - (1.96 * sem)]
  A_df[, band_hi := mean_val + (1.96 * sem)]
  ...
  pA <- ggplot(A_df, aes(x = Day, y = mean_val)) +
    # This checks the variable you defined in Step 1
    {if (error_style == "shading") geom_ribbon(aes(ymin = band_lo, ymax = band_hi), alpha = 0.25)} +
    {if (error_style == "bars") geom_errorbar(aes(ymin = band_lo, ymax = band_hi), width = 0.5)} +
    geom_line() +
    geom_point() +
    labs(
      title = "A",
  ...
  pB <- ggplot(B_df, aes(x = Day, y = disp)) +
    {if (error_style == "shading") geom_ribbon(aes(ymin = disp_lo, ymax = disp_hi), alpha = 0.25)} +
    {if (error_style == "bars") geom_errorbar(aes(ymin = disp_lo, ymax = disp_hi), width = 0.5)} +
    geom_line() +
    geom_point() +
    labs(
      title = "B",
...
qmd <- c(
  ...
  "  ok_ht <- requireNamespace('htmltools', quietly = TRUE)",
  "  if (!ok) {",
  "    cat('plotly is not available in this R session; falling back to static plot above.')",
  "  } else if (!ok_ht) {",
  "    cat('htmltools is not available in this R session; cannot emit htmlwidgets reliably.')",
  "  } else {",
  ...
  "        if (is.null(p)) {",
  "          widgets[[length(widgets) + 1]] <- htmltools::tags$p(paste0('Could not read plot RDS: ', basename(f)))",
  "        } else {",
  "          w <- tryCatch({",
  ...
  "          }, error = function(e) NULL)",
  "          if (is.null(w)) {",
  "            widgets[[length(widgets) + 1]] <- htmltools::tags$p(paste0('plotly rendering failed for: ', basename(f)))",
  "          } else {",
  "            widgets[[length(widgets) + 1]] <- htmltools::tagList(",
  "              htmltools::tags$h4(basename(f)),",
  "              w",
  "            )",
  "          }",
  "        }",
