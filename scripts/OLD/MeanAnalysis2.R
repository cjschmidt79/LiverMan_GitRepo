#!/usr/bin/env Rscript
# ======================================================================
#CURRENTLY SET TO LOG 2 TRANSFORM + PSEUDOCOUNT
#
#
...
# ======================================================================
# Manifest (mandatory): write initial manifest + file inventory (pre-render)
# ======================================================================
...
# ======================================================================
# Quarto QMD (mandatory) + render to HTML (final step)
# ======================================================================
...
qmd_lines <- c(
  ...
  "```{r}",
  "meta <- data.frame(",
  "  key = c('run_id','run_timestamp','script_name','script_path','script_full','input_csv','output_dir','runtime_seconds','enable_plotly','enable_runtime_tracking'),",
  "  value = c(params$run_id, params$run_timestamp, params$script_name, params$script_path, params$script_full, params$input_csv, params$output_dir, params$runtime_seconds, params$enable_plotly, params$enable_runtime_tracking),",
  "  stringsAsFactors = FALSE",
  ")",
  "rownames(meta) <- NULL",
  "knitr::kable(meta)",
  ...
  "dep_df <- man$dependencies",
  "if (is.null(dep_df) || length(dep_df) == 0) {",
  "  dep_df <- data.frame(package = character(0), version = character(0), stringsAsFactors = FALSE)",
  "} else {",
  "  dep_df <- as.data.frame(dep_df, stringsAsFactors = FALSE)",
  "}",
  "rownames(dep_df) <- NULL",
  "knitr::kable(dep_df, caption = 'R package dependencies (exact versions)')",
  "```",
  "",
  "# Generated files",
  "",
  "```{r}",
  "inv <- read.csv(file.path(params$output_dir, 'Project_Manifest_Files.csv'), stringsAsFactors = FALSE, check.names = FALSE)",
  "rownames(inv) <- NULL",
  "knitr::kable(inv, caption = 'Inventory of generated files in output_dir')",
  "```",
  "",
  "# Analytical logic and formulas",
  "",
  "This report summarizes mean trajectories and dispersion across post-hatch days from a tidy, replicate-level table.",
  "",
  "## Per-Source × Metabolite × Day mean and 95% CI",
  "",
  "- Mean:  \\( \\bar{x} = \\frac{1}{n}\\sum_i x_i \\)",
  "- SD:    \\( s = \\sqrt{\\frac{1}{n-1}\\sum_i (x_i-\\bar{x})^2} \\) (for \\(n\\ge 2\\))",
  "- SE:    \\( \\mathrm{SE} = s/\\sqrt{n} \\)",
  "- 95% CI (t-based):  \\( \\bar{x} \\pm t_{0.975, n-1}\\cdot \\mathrm{SE} \\)",
  "",
  "## Feature-level CV within day (biological replicates)",
  "",
  "- \\( \\mathrm{CV} = s/|\\bar{x}| \\) computed across biological replicates within a day for each metabolite.",
  "",
  "## Group-mean CV by day (within Source)",
  "",
  "- For each Source×Day, compute the mean of metabolite CVs and a t-based CI across metabolites.",
  "- The additional quantile band (IQR or P10–P90) is **dispersion across metabolites**, not uncertainty.",
  "",
  "## Global ALL-sources replicate “system mean” plot",
  "",
  "- For each Source×Day×SampleID, compute SystemMean = mean abundance across metabolites.",
  "- Across SampleIDs on each day (pooling Sources):",
  "  - Grey band = IQR or P10–P90 of SystemMean (inter-individual dispersion).",
  "  - CI bars = t-based 95% CI of the **mean** SystemMean (uncertainty on the mean).",
  "",
  "## Levene (median-centered) + permutation",
  "",
  "- For each Source×Metabolite, Levene is computed across days on replicate-level values using median centering.",
  "- Permutation p-value is computed by shuffling day labels within Source×Metabolite.",
  "",
  "# Key diagnostic plot (static)",
  "",
  "```{r}",
  "kp <- file.path(params$output_dir, 'KeyPlot_GlobalSystemMean.png')",
  "if (file.exists(kp)) {",
  "  knitr::include_graphics(kp)",
  "} else {",
  "  cat('KeyPlot_GlobalSystemMean.png not found in output_dir. (This is non-fatal.)\\n')",
  "}",
  "```",
  "",
  ...
  "  if (length(f) >= 1) {",
  "    gdf <- try(read.csv(f[1], stringsAsFactors = FALSE), silent = TRUE)",
  "    if (!inherits(gdf, 'try-error') && nrow(gdf) >= 2 && all(c('Day','Center') %in% names(gdf))) {",
  "      p <- ggplot(gdf, aes(x = Day, y = Center)) + geom_line() + geom_point() +",
  "        labs(title = 'Interactive: Global replicate system mean by day', x = 'Day', y = 'Center (mean/median)')",
  "      plotly::ggplotly(p)",
  "    } else {",
  "      cat('Interactive plot unavailable (insufficient data or missing columns).')",
  "    }",
  "  } else {",
  "    cat('Interactive plot unavailable (CSV not found).')",
  "  }",
  "} else {",
  "  cat('Interactive plots disabled by configuration.')",
  "}",
  "```",
  "",
  "# Interpretation guidance",
  "",
  "- The per-metabolite mean trajectories capture changes in average abundance over days.",
  "- The feature-level CV and its day-wise summaries capture within-day heterogeneity across biological replicates.",
  "- The ALL-sources replicate SystemMean plot separates (i) dispersion among individuals (grey band) from (ii) uncertainty on the day-wise mean (CI bars).",
  "",
  "# Runtime",
  "",
  "```{r}",
...
# -----------------------------
# Finalize runtime_seconds before render (mandatory if enabled)
# -----------------------------
runtime_seconds <- NA_real_
if (isTRUE(enable_runtime_tracking)) {
  end_time_pre_render <- Sys.time()
  runtime_seconds <- round(as.numeric(difftime(end_time_pre_render, start_time, units = "secs")), 2)
}

# Update manifest with runtime_seconds before render
manifest$parameters$runtime_seconds <- runtime_seconds
jsonlite::write_json(manifest, manifest_json_path, pretty = TRUE, auto_unbox = TRUE, na = "null")

# Render report as final step
render_ok <- TRUE
render_msg <- NULL
...
inv_final <- write_file_inventory(output_dir, manifest_files_csv)

# Update manifest again to reflect final file set + final runtime
manifest$parameters$runtime_seconds <- runtime_seconds
manifest$generated_files <- inv_final$rel_path
jsonlite::write_json(manifest, manifest_json_path, pretty = TRUE, auto_unbox = TRUE, na = "null")

# ======================================================================
# Done (console summary + report path)
# ======================================================================
cat("\nDone.\n")
cat("Output directory:\n  ", normalizePath(output_dir, winslash = "/", mustWork = FALSE), "\n", sep = "")
cat("Saved per-Source×Feature×Day table:\n  ", out_mean_byday, "\n", sep = "")
cat("Saved per-Source average-feature-mean by day table:\n  ", out_avg_byday, "\n", sep = "")
cat("Saved replicate system-mean table (Source×Day×SampleID):\n  ", out_rep_sys, "\n", sep = "")
cat("Saved global ALL-sources replicate system-mean by day table:\n  ", out_global, "\n", sep = "")
if (isTRUE(add_global_band)) cat("Saved ALL-sources replicate dispersion band table:\n  ", out_band, "\n", sep = "")

if (isTRUE(render_ok) && file.exists(html_full)) {
  cat("\nHTML report created:\n  ", normalizePath(html_full, winslash = "/", mustWork = FALSE), "\n", sep = "")
} else {
