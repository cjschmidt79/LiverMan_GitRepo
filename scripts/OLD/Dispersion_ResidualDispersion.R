...
# - Interactive file + column selection (RStudio pickers when available)
# - Analysis scale: (1) RAW or (2) log2(TPM + PC) with PC default = 1
# - Methods:
#     A) Within-gene dispersion across replicates per day (SD / IQR / MAD / CV*)
#     B) Residual dispersion (mean–variance trend removed within each day)
# - Outputs: ./outputs/<run_name>_<timestamp>/
# - Provenance: manifest.json + inventory_files.txt
# - Quarto HTML report (optional, if quarto package installed)
###############################################################################
...
analyze_gene_dispersion <- function(input_file = NULL,
  ...
  .pick_file <- function() {
    if (.is_rstudio()) {
      rstudioapi::selectFile(caption = "Select input CSV (tidy long table)")
    } else {
      file.choose()
    }
  }
  ...
  .get_script_identity <- function() {
    ...
    # Try RStudio editor context first (most common for SOURCE)
    if (.is_rstudio()) {
      try({
    ...
    # Fallback: commandArgs if run with Rscript
  ...
  day_disp_sd  <- summarize_day("sd")
  day_disp_iqr <- summarize_day("iqr")
  day_disp_mad <- summarize_day("mad")
  day_disp_cv  <- summarize_day("cv")
  
  day_disp_all <- rbind(day_disp_sd, day_disp_iqr, day_disp_mad, day_disp_cv)
  ...
  if (isTRUE(make_plots)) {
    if (requireNamespace("ggplot2", quietly = TRUE)) {
      ...
      f1 <- file.path(fig_dir, .safe_filename(paste0("Dispersion_ByDay_", scale_label, ".png")))
      grDevices::png(f1, width = 1800, height = 1100, res = 200)
      op <- par(mar = c(5, 5, 3, 1))
      dd <- day_disp_all[day_disp_all$metric %in% c("sd","iqr","mad"), ]
      metrics <- unique(dd$metric)
      
      plot(days, dd$median[dd$metric == metrics[1]], type = "b",
           xlab = "Day", ylab = "Median dispersion across genes",
           main = paste0("Within-gene dispersion (", scale_label, ")"))
      if (length(metrics) > 1) {
        for (m in metrics[-1]) lines(days, dd$median[dd$metric == m], type = "b")
      }
      legend("topright", legend = metrics, bty = "n")
      par(op); grDevices::dev.off()
      plot_paths$dispersion <- f1
      
      f2 <- file.path(fig_dir, .safe_filename(paste0("ResidualDispersion_ByDay_", scale_label, ".png")))
      grDevices::png(f2, width = 1800, height = 1100, res = 200)
      op <- par(mar = c(5, 5, 3, 1))
      plot(day_resid$Day, day_resid$median, type = "b",
           xlab = "Day", ylab = "Median residual log-variance",
           main = paste0("Residual dispersion (", scale_label, ")"))
      abline(h = 0, lty = 2)
      par(op); grDevices::dev.off()
      plot_paths$residual_dispersion <- f2
    }
  ...
  if (isTRUE(make_quarto) && requireNamespace("quarto", quietly = TRUE)) {
    ...
    qmd <- c(
      ...
      "## Summary",
      "",
      "This report quantifies (i) within-gene dispersion across biological replicates per day, and",
      "(ii) mean–variance-corrected residual dispersion within each day.",
      "",
      "## Metadata",
      "",
      "```{r}",
      "params <- list(",
      paste0("  timestamp = \"", .json_escape(timestamp), "\","),
      paste0("  run_name = \"", .json_escape(run_name), "\","),
      ...
      paste0("  transform = \"", .json_escape(transform_label), "\""),
      ")",
      "print(params)",
      "```",
      "",
      ...
      "print(list.files(tab_dir, full.names = TRUE))",
      "```",
      "",
      "```{r}",
      paste0("wide <- read.csv(\"Tables/", basename(out_wide_day), "\", stringsAsFactors = FALSE)"),
      "print(wide)",
      "```",
      "",
      "## Figures",
      ...
      "imgs <- list.files(fig_dir, pattern = \"\\\\.png$\", full.names = TRUE)",
      "for (p in imgs) { cat(\"![](\", p, \")\\n\\n\", sep = \"\") }",
      "```",
      "",
      "## Generated outputs",
    ...
    # Canonical contract: setwd(out_dir), render basename only, restore immediately
    old_wd <- getwd()
    setwd(out_dir)
  ...
  if (interactive()) {
    ...
    .safe_cat(out_dir)
    if (!is.null(result$tables$per_day_wide) && file.exists(result$tables$per_day_wide)) {
      try(utils::View(utils::read.csv(result$tables$per_day_wide, stringsAsFactors = FALSE)), silent = TRUE)
    }
  }
