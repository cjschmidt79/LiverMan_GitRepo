...
analyze_gene_dispersion <- function(input_file = NULL,
                                    ...
                                    make_quarto = TRUE,
                                    make_plots = TRUE,
                                    # ---------------- Variance driver extraction (AUTO) ----------------
                                    driver_metric = c("sd","var","iqr","mad","cv","resid_log_var"),
                                    top_n = 25,
                                    min_mean = 1.0,
                                    min_iqr = 0,
                                    auto_all_days = TRUE,
                                    auto_consecutive_delta = TRUE) {
  ...
  driver_metric <- match.arg(driver_metric)
  ...
  if (interactive()) {
    ...
    # ---- NEW: driver settings (optional) ----
    driver_metric <- .select_from(c("sd","var","iqr","mad","cv","resid_log_var"),
                                  "Choose driver metric (genes ranked within each day):",
                                  default = 1L)   # default sd
    
    tn_in <- .readline_default("Top N drivers to write per day", as.character(top_n))
    top_n <- suppressWarnings(as.integer(tn_in))
    if (!is.finite(top_n) || top_n < 5) top_n <- 25
    
    mm_in <- .readline_default("Filter drivers: minimum mean expression", as.character(min_mean))
    min_mean <- suppressWarnings(as.numeric(mm_in))
    if (!is.finite(min_mean) || min_mean < 0) min_mean <- 1.0
    
    mi_in <- .readline_default("Filter drivers: minimum IQR (0 keeps all)", as.character(min_iqr))
    min_iqr <- suppressWarnings(as.numeric(mi_in))
    if (!is.finite(min_iqr) || mi_in < 0) min_iqr <- 0
  ...
  # ------------------ C) Variance drivers (AUTO across days + consecutive Δ) --
  driver_metric <- match.arg(driver_metric)
  top_n <- as.integer(top_n)
  if (!is.finite(top_n) || top_n < 5) top_n <- 25
  
  .rank_drivers_for_day <- function(day_val, metric, min_mean, min_iqr, top_n) {
    subD <- per_gene_day[per_gene_day$Day == day_val, , drop = FALSE]
    
    keep <- is.finite(subD$mean) & is.finite(subD$iqr) & is.finite(subD$var)
    keep <- keep & (subD$mean >= min_mean) & (subD$iqr >= min_iqr)
    keep <- keep & is.finite(subD[[metric]])
    
    subD <- subD[keep, , drop = FALSE]
    if (nrow(subD) == 0) return(subD)
    
    ord <- order(subD[[metric]], decreasing = TRUE, na.last = NA)
    subD <- subD[ord, , drop = FALSE]
    subD$rank <- seq_len(nrow(subD))
    
    subD <- subD[seq_len(min(top_n, nrow(subD))), , drop = FALSE]
    subD$driver_metric <- metric
    subD$driver_type <- "top_within_day"
    subD
  }
  
  .rank_consecutive_delta <- function(day_prev, day_cur, metric, min_mean, min_iqr, top_n) {
    A <- per_gene_day[per_gene_day$Day == day_prev, c("Gene","mean","iqr", metric), drop = FALSE]
    B <- per_gene_day[per_gene_day$Day == day_cur,  c("Gene","mean","iqr", metric), drop = FALSE]
    
    m <- merge(A, B, by = "Gene", suffixes = c("_prev", "_cur"), all = FALSE)
    
    keep <- is.finite(m$mean_cur) & is.finite(m$iqr_cur)
    keep <- keep & (m$mean_cur >= min_mean) & (m$iqr_cur >= min_iqr)
    keep <- keep & is.finite(m[[paste0(metric, "_prev")]]) & is.finite(m[[paste0(metric, "_cur")]])
    
    m <- m[keep, , drop = FALSE]
    if (nrow(m) == 0) return(m)
    
    m$delta <- m[[paste0(metric, "_cur")]] - m[[paste0(metric, "_prev")]]
    
    ord <- order(m$delta, decreasing = TRUE, na.last = NA)
    m <- m[ord, , drop = FALSE]
    m$rank <- seq_len(nrow(m))
    
    m <- m[seq_len(min(top_n, nrow(m))), , drop = FALSE]
    m$Day_prev <- day_prev
    m$Day_cur  <- day_cur
    m$driver_metric <- metric
    m$driver_type <- "top_consecutive_delta"
    
    front <- c("Day_prev","Day_cur","rank","Gene","delta",
               paste0(metric,"_prev"), paste0(metric,"_cur"),
               "mean_prev","mean_cur","iqr_prev","iqr_cur",
               "driver_metric","driver_type")
    rest <- setdiff(colnames(m), front)
    m <- m[, c(front, rest), drop = FALSE]
    m
  }
  
  driver_by_day_tbl <- data.frame()
  out_driver_by_day <- NA_character_
  
  if (isTRUE(auto_all_days)) {
    rows <- list()
    for (d in days) {
      td <- .rank_drivers_for_day(d, driver_metric, min_mean, min_iqr, top_n)
      if (nrow(td) > 0) rows[[as.character(d)]] <- td
    }
    if (length(rows) > 0) driver_by_day_tbl <- do.call(rbind, rows)
  }
  
  if (nrow(driver_by_day_tbl) > 0) {
    fn <- .safe_filename(paste0("VarianceDrivers_Top", top_n, "_ByDay_", driver_metric, "_", scale_label, ".csv"))
    out_driver_by_day <- .write_csv(driver_by_day_tbl, file.path(tab_dir, fn))
    .safe_cat("")
    .safe_cat("Top drivers per day written to:")
    .safe_cat("  ", out_driver_by_day)
  }
  
  delta_tbl <- data.frame()
  out_driver_consecutive <- NA_character_
  
  if (isTRUE(auto_consecutive_delta) && length(days) >= 2) {
    drows <- list()
    for (i in 2:length(days)) {
      d0 <- days[i - 1]
      d1 <- days[i]
      dd <- .rank_consecutive_delta(d0, d1, driver_metric, min_mean, min_iqr, top_n)
      if (nrow(dd) > 0) drows[[paste0(d0, "_", d1)]] <- dd
    }
    if (length(drows) > 0) delta_tbl <- do.call(rbind, drows)
  }
  
  if (nrow(delta_tbl) > 0) {
    fn <- .safe_filename(paste0("VarianceDrivers_Top", top_n, "_ConsecutiveDelta_", driver_metric, "_", scale_label, ".csv"))
    out_driver_consecutive <- .write_csv(delta_tbl, file.path(tab_dir, fn))
    .safe_cat("")
    .safe_cat("Top consecutive-day delta drivers written to:")
    .safe_cat("  ", out_driver_consecutive)
  }
  ...
  manifest <- list(
    ...
    min_mean = min_mean,
    min_iqr = min_iqr,
    auto_all_days = auto_all_days,
    auto_consecutive_delta = auto_consecutive_delta
    )
  ...
  result <- list(
    ...
    tables = list(
      ...
      variance_drivers_by_day = out_driver_by_day,
      variance_drivers_consecutive_delta = out_driver_consecutive
    ),
    figures = plot_paths,
  ...
  if (interactive()) {
    ...
    res <- analyze_gene_dispersion(driver_metric = "sd", top_n = 25)
    message("Done. Results written to:")
    message(res$out_dir)
    
    if (!is.null(res$tables$variance_drivers_by_day) &&
        file.exists(res$tables$variance_drivers_by_day)) {
      try(View(read.csv(res$tables$variance_drivers_by_day, stringsAsFactors = FALSE)), silent = TRUE)
    }
  }
