...
#   UPDATE: runtime params + plotly + show first 30 rows of all CSV outputs
# -----------------------------
...
# ADD THIS BLOCK HERE
# =============================
eff_mat <- as.data.table(boot_vals$eff_dim)
data.table::setnames(eff_mat, as.character(days_vec))
eff_mat[, b := 1:.N]
data.table::setcolorder(eff_mat, c("b", as.character(days_vec)))

out_eff_boot <- file.path(output_dir, "Bootstrap_EffDim_ByDay.csv")
data.table::fwrite(eff_mat, out_eff_boot)
register_output(out_eff_boot)
logf("Wrote Bootstrap_EffDim_ByDay.csv (replicate-level eff_dim by day)")

# ------------------------------------------------------------
# Save Bootstrap_EffDim_ByDay.csv (one extra file)
# Purpose: expose bootstrap replicate-level effective dimensionality per day
# Columns: b (bootstrap id), Day, eff_dim
# ------------------------------------------------------------
...
if (!is.matrix(eff_mat) || nrow(eff_mat) != B_TOTAL) {
  ...
  # Ensure day columns are the actual day labels (as characters)
...
write_transition_driver_tables_all_pairs <- function(meanM, days_vec, output_dir,
  ...
  for (i in 2:length(days_vec)) {
    ...
    register_output(out_path)  # keep your manifest accurate
  }
}

# Call it after meanM is created:
write_transition_driver_tables_all_pairs(meanM, days_vec, output_dir, top_pos = 200L, top_neg = 200L)

# NEW: transition driver tables for every adjacent day pair
write_transition_driver_tables_all_pairs(meanM, days_vec, output_dir, top_pos = 200L, top_neg = 200L)
...
#   UPDATE: runtime_seconds, enable_plotly, enable_runtime_tracking in parameters
# -----------------------------
...
# Runtime end/seconds computed just before render (MANDATORY if enabled)
...
manifest$generated_files <- split(
  inv_dt[, .(relative_path, full_path, bytes, modified)],
  seq_len(nrow(inv_dt))
)
