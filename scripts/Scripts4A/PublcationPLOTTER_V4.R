...
# ---------------------------- Plot Constructors (PCA/XY unchanged) ----------------------------
...
# ---------------------------- NEW: timecourse/summary/hist constructors ----------------------------

detect_ci_triplet <- function(df, y_col) {
  # If user picked "mid/mean/value", try to auto-find bounds among common names
  nm <- names(df)
  canon <- tolower(nm)
  
  y_lc <- tolower(y_col)
  # common patterns for "mid" naming
  mid_like <- c("mid", "mean", "value", "estimate", "est", "y", "median")
  lo_like  <- c("lo", "low", "lower", "lwr", "ci_lo", "cil", "min")
  hi_like  <- c("hi", "high", "upper", "upr", "ci_hi", "cih", "max")
  
  has_mid <- any(grepl(paste0("^(", paste(mid_like, collapse = "|"), ")$"), y_lc))
  # If the chosen y name isn't exactly mid/mean, we still attempt matching by presence of lo/hi cols
  lo_hit <- nm[canon %in% lo_like]
  hi_hit <- nm[canon %in% hi_like]
  
  if (length(lo_hit) >= 1 && length(hi_hit) >= 1) {
    return(list(lo = lo_hit[1], hi = hi_hit[1]))
  }
  if (has_mid) {
    # sometimes bounds are "lo"/"hi" while mid is named mid/mean
    lo2 <- nm[grepl("\\blo\\b|\\blower\\b|\\blwr\\b|\\bci.*lo\\b", canon)]
    hi2 <- nm[grepl("\\bhi\\b|\\bupper\\b|\\bupr\\b|\\bci.*hi\\b", canon)]
    if (length(lo2) >= 1 && length(hi2) >= 1) return(list(lo = lo2[1], hi = hi2[1]))
  }
  NULL
}
...
plot_timecourse <- function(df_raw, x_col, y_col,
  ...
  # ribbon if bounds provided
  if (!is.null(lo_col) && !is.null(hi_col)) {
    if (!is.null(group_col)) {
      p <- p +
        geom_ribbon(aes(ymin = .data[[lo_col]], ymax = .data[[hi_col]], fill = .data[[group_col]]),
      ...
      p <- p +
        geom_ribbon(aes(ymin = .data[[lo_col]], ymax = .data[[hi_col]]),
                    alpha = ribbon_alpha, fill = "grey70", color = NA)
    }
  }
  ...
  if (show_points) {
    p <- p + geom_point(alpha = point_alpha, size = 2.0)
  }
...
plot_summary_interval <- function(df_raw, x_col, y_col,
                                  lo_col = NULL, hi_col = NULL,
                                  group_col = NULL, facet_col = NULL,
                                  title = NULL, xlab = NULL, ylab = NULL,
                                  legend_pos = "right",
                                  geom_style = c("point", "bar"),
                                  point_size = 2.4,
                                  group_palette = "default") {
  ...
  if (geom_style == "bar") {
    if (!is.null(group_col)) {
      p <- p + geom_col(aes(fill = .data[[group_col]]), position = position_dodge(width = 0.8), width = 0.7, alpha = 0.9)
      p <- apply_group_palette(p, group_palette, group_is_fill = TRUE)
    } else {
      p <- p + geom_col(width = 0.7, alpha = 0.9)
    }
  } else {
  ...
  if (!is.null(group_col) && geom_style != "bar") {
    p <- apply_group_palette(p, group_palette, group_is_fill = FALSE)
  }
  ...
  p +
    labs(title = title %||% "Summary (with intervals)",
         x = xlab %||% x_col,
         y = ylab %||% y_col) +
    theme_pub(legend_pos = legend_pos)
}

plot_histogram <- function(df_raw, x_col,
                           group_col = NULL, facet_col = NULL,
                           title = NULL, xlab = NULL, ylab = NULL,
                           legend_pos = "right",
                           bins = 30,
  ...
  df_raw[[x_col]] <- coerce_numeric_if_reasonable(df_raw[[x_col]])
  if (!is.numeric(df_raw[[x_col]])) stop2("Histogram column '", x_col, "' is not numeric (or could not be safely coerced).")
  
  if (!is.null(group_col)) {
    p <- ggplot(df_raw, aes(x = .data[[x_col]], fill = .data[[group_col]])) +
      geom_histogram(bins = bins, alpha = 0.75, position = "identity")
    p <- apply_group_palette(p, group_palette, group_is_fill = TRUE)
  } else {
    p <- ggplot(df_raw, aes(x = .data[[x_col]])) +
      geom_histogram(bins = bins, alpha = 0.85)
  }
  
  if (show_mean || show_ci) {
    xvals <- df_raw[[x_col]]
    xvals <- xvals[is.finite(xvals)]
    if (length(xvals) >= 2) {
      mu <- mean(xvals)
      se <- sd(xvals) / sqrt(length(xvals))
      ci <- mu + c(-1, 1) * qt(0.975, df = max(1, length(xvals) - 1)) * se
      if (show_mean) p <- p + geom_vline(xintercept = mu, linewidth = 0.8, linetype = "solid")
      if (show_ci) {
        p <- p + geom_vline(xintercept = ci[1], linewidth = 0.7, linetype = "dashed")
        p <- p + geom_vline(xintercept = ci[2], linewidth = 0.7, linetype = "dashed")
      }
    }
  }
...
if (plot_type == "timecourse") {
  ...
  x_col <- choose_column(df, "Select X column (e.g., Day):", show_stats = TRUE)
  df[[x_col]] <- coerce_numeric_if_reasonable(df[[x_col]])
  require_not_constant(df, x_col, label = "X column")
  
  y_col <- choose_column(df, "Select Y column (plotted value):", show_stats = TRUE)
  ...
  # bounds: optional; if user says "yes", we auto-detect lo/hi first
  lo_col <- NULL
  hi_col <- NULL
  use_bounds <- choose_yes_no("Do you have precomputed interval bounds (lo/hi) for a ribbon?", default = "n")
  if (use_bounds) {
    auto <- detect_ci_triplet(df, y_col = y_col)
    if (!is.null(auto)) {
      cat("\nDetected bounds: lo='", auto$lo, "' hi='", auto$hi, "'\n", sep = "")
      ok <- choose_yes_no("Use these bounds?", default = "y")
      if (ok) {
        lo_col <- auto$lo
        hi_col <- auto$hi
      }
    ...
    if (is.null(lo_col) || is.null(hi_col)) {
      lo_col <- choose_column(df, "Select LOWER bound column (lo):", show_stats = TRUE)
      hi_col <- choose_column(df, "Select UPPER bound column (hi):", show_stats = TRUE)
      df[[lo_col]] <- coerce_numeric_if_reasonable(df[[lo_col]])
      df[[hi_col]] <- coerce_numeric_if_reasonable(df[[hi_col]])
      if (!is.numeric(df[[lo_col]]) || !is.numeric(df[[hi_col]])) stop2("Bounds must be numeric.")
    }
  }
  ...
  show_points <- choose_yes_no("Show points on top of the line?", default = "y")
  ...
  print(p)
  stem <- paste0("TIMECOURSE_", safe_stem(x_col), "_", safe_stem(y_col), "_", ts_stamp())
  ...
  x_col <- choose_column(df, "Select X column (category or Day):", show_stats = TRUE)
  require_not_constant(df, x_col, label = "X column")
  
  y_col <- choose_column(df, "Select Y column (summary value):", show_stats = TRUE)
  df[[y_col]] <- coerce_numeric_if_reasonable(df[[y_col]])
  if (!is.numeric(df[[y_col]])) stop2("Y column '", y_col, "' is not numeric (or could not be safely coerced).")
  require_not_constant(df, y_col, label = "Y column")
  ...
  lo_col <- NULL
  hi_col <- NULL
  use_bounds <- choose_yes_no("Do you have precomputed interval bounds (lo/hi) for error bars?", default = "y")
  if (use_bounds) {
    auto <- detect_ci_triplet(df, y_col = y_col)
    if (!is.null(auto)) {
      cat("\nDetected bounds: lo='", auto$lo, "' hi='", auto$hi, "'\n", sep = "")
  ...
  group_col <- choose_optional_column(df, "Select GROUP column (optional; color/fill):", show_stats = TRUE)
  facet_col <- choose_optional_column(df, "Select FACET column (optional):", show_stats = TRUE)
  
  cat("\nSummary style:\n  [1] point + interval\n  [2] bar + interval\n")
  st <- suppressWarnings(as.integer(readline("Enter number [1]: ")))
  if (is.na(st) || !(st %in% 1:2)) st <- 1
  geom_style <- c("point", "bar")[st]
  ...
  print(p)
  stem <- paste0("SUMMARY_", safe_stem(x_col), "_", safe_stem(y_col), "_", ts_stamp())
  paths <- save_pub(p, out_dir, stem, width = width, height = height)
  htmlp <- save_plotly_html(p, out_dir, stem, width_in = width, height_in = height)
  plotly_files <- c(plotly_files, htmlp)
  ...
  x_col <- choose_column(df, "Select numeric column for histogram:", show_stats = TRUE)
  df[[x_col]] <- coerce_numeric_if_reasonable(df[[x_col]])
  if (!is.numeric(df[[x_col]])) stop2("Histogram column '", x_col, "' is not numeric (or could not be safely coerced).")
  require_not_constant(df, x_col, label = "Histogram column")
  
  group_col <- choose_optional_column(df, "Select GROUP column (optional; fills by group):", show_stats = TRUE)
  facet_col <- choose_optional_column(df, "Select FACET column (optional):", show_stats = TRUE)
  
  bins <- suppressWarnings(as.integer(readline("Number of bins [30]: ")))
  if (is.na(bins) || bins < 1) bins <- 30
  
  show_mean <- choose_yes_no("Add mean line?", default = "y")
  show_ci <- choose_yes_no("Add 95% CI lines (mean ± t*SE)?", default = "n")
  ...
  print(p)
  stem <- paste0("HIST_", safe_stem(x_col), "_", ts_stamp())
  paths <- save_pub(p, out_dir, stem, width = width, height = height)
  htmlp <- save_plotly_html(p, out_dir, stem, width_in = width, height_in = height)
  plotly_files <- c(plotly_files, htmlp)
  ...
  stop2("Unhandled plot type: ", plot_type)
}
