...
#        - "off"/near-zero genes excluded from that regression per day
# 4) Evaluates trajectory structure across the ENTIRE experiment:
#    Layer 1 (PRIMARY): Global axis projection (robust to loops / degenerate endpoints)
#    Layer 2: Tangent field continuity for every adjacent interval
#    Layer 3: Piecewise regime vectors + (recommended) sliding-window coherence
# 5) Bias controls:
...
#    - Quarto QMD + HTML report (if Quarto + executable found)
#
# Requirements:
# - CSV input only
...
trajectory_vector_geometry_wide <- function(input_file = NULL,
  ...
  # ------------------- NA-robust vector math (CRITICAL FIX) -------------------
  dot <- function(a, b) {
  ...
  say("📁 Select OUTPUT parent folder (script creates outputs/<run>_<timestamp>/ under it).")
  say("   Tip: Cancel the picker to use default ./outputs under current working directory.")
  out_parent <- pick_dir()
  ...
  if (any(is.na(rep_parsed))) {
    say("⚠️ Replicate index (after underscore) could not be parsed for some rows. OK unless you need Rep explicitly.")
  }
  ...
  if (!is.na(day_col)) {
    ...
    if (length(mismatch) > 0) {
      stop("Day column does not match Day parsed from SampleID. Fix naming or choose NONE for Day column.")
    }
    day <- day_given
  } else {
    day <- day_parsed
  ...
  if (transform == "log2p1") {
    say("🔁 Applying log2(x + pseudocount) with pseudocount=", pseudocount)
  ...
  compute_day_vectors <- function(Xmat, day_vec, group_vec, group_value, eps_var = 1e-8) {
    ...
    say("Days in analysis: ", paste(days, collapse = ", "))
    ...
    for (d in days) {
      ...
      if (nrow(Xd) >= 2) {
        ...
        # exclude genes with zero/near-zero mean from mean–variance regression
        mean_ok <- abs(mu) > eps_var
        ok <- is.finite(logvar) & is.finite(logmu) & mean_ok
  ...
  eval_layers <- function(days, x_by_day, label,
                          pca_k = 20,
                          pca_train_max_day = 14,
                          bootstrap_CI = NULL,
                          permute_P = 0,
    ...
    # build day matrix (D x G_raw)
    D <- length(days)
    day_names <- as.character(days)
    G_raw <- length(x_by_day[[day_names[1]]])
    
    Xday <- matrix(NA_real_, nrow = D, ncol = G_raw)
    for (i in seq_along(days)) {
      Xday[i, ] <- x_by_day[[as.character(days[i])]]
    }
    
    # Consistent gene set across days (critical for DISP)
    keep_genes <- colSums(is.finite(Xday)) == nrow(Xday)
    if (sum(keep_genes) < 50) {
      keep_genes <- colSums(is.finite(Xday)) >= ceiling(0.80 * nrow(Xday))
    }
    if (sum(keep_genes) < 10) {
      stop(label, ": too few genes remain after NA filtering across days. ",
           "This usually means residual dispersion is mostly NA (insufficient expressed genes / too few reps).")
    }
    
    Xday <- Xday[, keep_genes, drop = FALSE]
    D <- nrow(Xday)
    G <- ncol(Xday)
    
    na_frac <- sum(!is.finite(Xday)) / length(Xday)
    if (na_frac > 0.10) {
      say(sprintf("⚠️ %s day-matrix has %.2f%% non-finite entries after filtering.", label, 100*na_frac))
    }
    ...
    vgn <- vnorm(v_global)
    axis_def <- paste0(days[1], "→", days[D])
    
    if (!is.finite(vgn) || vgn < 1e-10) {
      dist_mat <- as.matrix(stats::dist(Xday))
      ij <- which(dist_mat == max(dist_mat, na.rm = TRUE), arr.ind = TRUE)[1, ]
      ii <- ij[1]; jj <- ij[2]
      v_global <- Xday[jj, ] - Xday[ii, ]
      vgn <- vnorm(v_global)
      axis_def <- paste0(days[ii], "→", days[jj])
    }
    
    axis_degenerate <- (!is.finite(vgn) || vgn < 1e-10)
    if (axis_degenerate) {
      say("⚠️ ", label, ": global axis degenerate (looping / net displacement ~0). ",
          "Global projection metrics set to NA. Tangent continuity + step magnitudes remain valid.")
      vhat <- rep(NA_real_, G)
    } else {
      vhat <- v_global / vgn
    }
    ...
    for (i in seq_len(n_int)) {
      ...
      if (axis_degenerate) {
        cos_to_global[i] <- NA_real_
        along_s[i] <- NA_real_
        orth_norm[i] <- NA_real_
      } else {
        cos_to_global[i] <- cosine(delta, v_global)
        s <- dot(delta, vhat)
    ...
    # Permutation p-values (USE FILTERED Xday — avoids length mismatch)
    perm_p <- rep(NA_real_, n_int)
    if (permute_P > 0 && !axis_degenerate) {
    ...
    # Break percentiles (robust to NAs)
    if (all(!is.finite(tab$orth_norm))) {
      tab$orth_break_percentile <- NA_real_
    } else {
      orth_rank <- rank(-tab$orth_norm, ties.method = "average", na.last = "keep")
      denom <- (sum(is.finite(orth_rank)) - 1)
      tab$orth_break_percentile <- if (denom > 0) (orth_rank - 1) / denom else NA_real_
    }
    
    tab$tangent_break_percentile <- NA_real_
    ok_t <- is.finite(tab$tangent_continuity)
    if (sum(ok_t) >= 2) {
      r <- rank(tab$tangent_continuity[ok_t], ties.method = "average")
      denom <- (length(r) - 1)
      tang_break_pct <- if (denom > 0) 1 - ((r - 1) / denom) else rep(NA_real_, length(r))
      tab$tangent_break_percentile[ok_t] <- tang_break_pct
    }
    
    # PCA replication (fixed-basis)
    pca_res <- NULL
    try({
    ...
    # attach bootstrap CI columns (bootstrap_CI is a data.frame from bootstrap_interval_CI)
    ...
    if (isTRUE(make_plots)) {
      ...
      # Along vs Orth (may be NA if axis degenerate; still plots)
      ...
      plot(tab$along_s, type = "b", pch = 16, xaxt = "n",
           xlab = "", ylab = "Value",
           main = paste0(label, ": Along-axis progress vs Orthogonal excursion"))
      axis(1, at = seq_along(xlab), labels = xlab, las = 2, cex.axis = 0.8)
      lines(tab$orth_norm, type = "b", pch = 1)
      legend("topright", legend = c("along_s", "orth_norm"), bty = "n", pch = c(16,1))
      grDevices::dev.off()
      ...
      if (!is.null(pca_res) && !is.null(pca_res$scores) && ncol(pca_res$scores) >= 2) {
        ...
        for (i in 1:(nrow(sc)-1)) arrows(sc[i,1], sc[i,2], sc[i+1,1], sc[i+1,2], length = 0.08)
        grDevices::dev.off()
  ...
  bootstrap_interval_CI <- function(days, day_vec, group_vec, group_value, Xmat,
    ...
    for (b in seq_len(B)) {
      ...
      groupb <- rep(group_value, length(sampled_idx))
      
      vb <- compute_day_vectors(Xb, dayb, groupb, groupb, eps_var = eps_var)
      ...
      # SAME gene-consistency filter used in eval_layers
      keep <- colSums(is.finite(Xday)) == nrow(Xday)
      if (sum(keep) < 50) keep <- colSums(is.finite(Xday)) >= ceiling(0.80 * nrow(Xday))
      if (sum(keep) < 10) next
      Xday <- Xday[, keep, drop = FALSE]
      
      vG <- Xday[D, ] - Xday[1, ]
      vgN <- vnorm(vG)
      if (!is.finite(vgN) || vgN < 1e-12) next
      vhat <- vG / vgN
  ...
  piecewise_regimes <- function(days, x_by_day, boundaries) {
    closest_day <- function(target) days[which.min(abs(days - target))]
  ...
  sliding_window_coherence <- function(days, x_by_day, win_points = 4) {
    if (!is.finite(win_points) || win_points < 3) stop("win_points must be an integer >= 3")
    D <- length(days)
    if (D < win_points) return(NULL)
    
    rows <- list()
    k <- 1
    
    for (i in 1:(D - win_points + 1)) {
      w_days <- days[i:(i + win_points - 1)]
      start <- w_days[1]
      end <- w_days[length(w_days)]
      
      v_win <- x_by_day[[as.character(end)]] - x_by_day[[as.character(start)]]
      vn <- vnorm(v_win)
      if (!is.finite(vn) || vn < 1e-12) next
      
      cs <- c()
      for (j in 1:(length(w_days) - 1)) {
        d1 <- w_days[j]
        d2 <- w_days[j + 1]
        step <- x_by_day[[as.character(d2)]] - x_by_day[[as.character(d1)]]
        cs <- c(cs, cosine(step, v_win))
      }
      
      rows[[k]] <- data.frame(
        window = paste0(start, "_to_", end),
        start = start,
        end = end,
        midpoint = (start + end) / 2,
        n_steps = length(w_days) - 1,
        mean_within_alignment = mean(cs, na.rm = TRUE),
        median_within_alignment = stats::median(cs, na.rm = TRUE),
        min_step_alignment = min(cs, na.rm = TRUE),
        stringsAsFactors = FALSE
      )
      k <- k + 1
    }
  ...
  use_sliding <- readline("Use sliding (overlapping) windows for coherence? (y/n) [default y]: ")
  use_sliding <- !(nzchar(use_sliding) && tolower(trimws(use_sliding)) %in% c("n","no"))
  
  win_points_in <- readline("Sliding window size in DAY-POINTS (4 gives 3 steps; default 4): ")
  win_points <- suppressWarnings(as.integer(win_points_in))
  if (!is.finite(win_points) || win_points < 3) win_points <- 4
  
  manifest$use_sliding_windows <- use_sliding
  manifest$sliding_window_points <- win_points
  
  default_bounds <- c(4, 10, 14, 20)
  b_in <- readline(paste0("Optional fixed regime boundaries (comma-separated; default ",
  ...
  for (gval in groups_to_run) {
    ...
    if (length(bad_days) > 0) {
      say("⚠️ Days with <2 birds in group '", gval, "': ", paste(bad_days, collapse = ", "),
          " — dispersion/residual variance may be sparse for those days.")
    }
    ...
    # ---------------- Coherence outputs ----------------
    slide_mean_path <- NA_character_
    slide_disp_path <- NA_character_
    
    if (isTRUE(use_sliding)) {
      sw_mean <- sliding_window_coherence(days, vecs$mean_by_day, win_points = win_points)
      sw_disp <- sliding_window_coherence(days, vecs$disp_resid_by_day, win_points = win_points)
      
      if (!is.null(sw_mean)) {
        slide_mean_path <- file.path(dir_tables, paste0("SlidingWindow_Coherence_", prefix_mean, "_W", win_points, ".csv"))
        utils::write.csv(sw_mean, slide_mean_path, row.names = FALSE)
      }
      if (!is.null(sw_disp)) {
        slide_disp_path <- file.path(dir_tables, paste0("SlidingWindow_Coherence_", prefix_disp, "_W", win_points, ".csv"))
        utils::write.csv(sw_disp, slide_disp_path, row.names = FALSE)
      }
    }
    ...
    results_by_group[[gval]] <- list(
      ...
      mean = res_mean,
      disp = res_disp,
      sliding_mean_path = slide_mean_path,
      sliding_disp_path = slide_disp_path,
  ...
  if (isTRUE(make_quarto)) {
    have_quarto <- FALSE
    if (requireNamespace("quarto", quietly = TRUE) && nzchar(Sys.which("quarto"))) have_quarto <- TRUE
    if (!have_quarto) say("⚠️ Quarto not available on PATH (or quarto R package missing) — skipping HTML render.")
  ...
  say("Log: ", log_file)
  if (file.exists(html_path)) say("HTML report: ", html_path) else say("HTML report not found.")
...
if (interactive()) {
  ...
  if (!is.null(res$results_by_group)) {
    ...
    if (file.exists(tab_try)) {
      ...
      if (!inherits(tt, "try-error")) try(View(tt), silent = TRUE)
    }
  }
}
