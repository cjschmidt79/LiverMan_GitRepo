...
# Apply transform (NEW) into a dedicated column Abundance_use
apply_transform <- function(x, mode, pseudocount) {
  if (mode == "raw") return(x)
  # log2(Abundance + pseudocount)
  # Guard: if any x + pseudocount <= 0, result is -Inf/NaN; those propagate to NA after is.finite checks later.
  log2(x + pseudocount)
}

df <- df %>%
  mutate(Abundance_use = apply_transform(Abundance, transform_mode, pseudocount))

if (transform_mode == "log2p") {
  # Advisory only; do not abort unless everything becomes non-finite.
  bad_n <- sum(!is.finite(df$Abundance_use) & !is.na(df$Abundance_use))
  if (bad_n > 0) {
    message("NOTE: ", bad_n, " values became non-finite after log2 transform (likely Abundance + pseudocount <= 0). These will be dropped by finite/complete-case filtering.")
  }
}
...
build_sample_by_gene_matrix <- function(df_in) {
  ...
  # Drop genes with any NA across samples for THIS matrix
  ...
  if (any(!is.finite(X))) {
    ...
    # Remove genes with any NA after non-finite handling
    keep_cols <- apply(X, 2, function(v) all(!is.na(v)))
...
for (i in seq_len(nrow(day_pairs))) {
  ...
  if (is.null(X_joint)) {
    message("Skipping pair: joint PCA matrix could not be built after filtering (too many NA/non-finite or too few genes).")
    next
  }
