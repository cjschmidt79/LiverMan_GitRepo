...
#
# COMPLETE UPDATED SCRIPT (PART 1 OF 3)
#   - Cleans up duplicated blocks
#   - Adds ONE correct Bootstrap_EffDim_ByDay.csv writer (long format) [in Part 2]
#   - Ensures transition-driver tables are called ONCE [in Part 2]
#   - QMD/HTML remains rmarkdown::render-based and auto-includes CSV previews [Part 3]
#
# PURPOSE
#   Summarize how transcriptome-wide variability and coordination change across
#   a small number of developmental timepoints ("small T", e.g., ~9 days).
#
# OUTPUTS (under outputs/<run_id>/)
#   - Table_1_Daily_Metrics.csv
#   - Table_1b_Daily_Metrics_BootstrapCI.csv
#   - Bootstrap_EffDim_ByDay.csv                (NEW; bootstrap replicate-level eff_dim by day; long format)
#   - Bootstrap_Raw_AllStats.csv
#   - Table_2..Table_5 bootstrap support tables
#   - Table_6 driver genes for canalization day (if identified)
#   - Table_7 driver genes for each adjacent day pair (balanced up/down)
#   - Plots/Metric_Trends_Annotated.png
...
suppressPackageStartupMessages({
  ...
  if (isTRUE(enable_plotly)) pkgs <- unique(c(pkgs, "plotly", "htmlwidgets"))
...
# If the script filename is known, set it here as a fallback (ONLY affects labeling)
known_script_filename <- "TemporalStructure.R"
...
write_files_inventory <- function(output_dir, inventory_csv_path) {
  ...
  rel <- sub(
    paste0("^", gsub("\\\\", "/", normalizePath(output_dir, winslash="/", mustWork=FALSE)), "/?"),
    "",
    gsub("\\\\", "/", files)
  )
...
# =========================
# END PART 1 OF 3
# Next: report writer + main execution + SINGLE effdim writer + SINGLE transition-driver call
# =========================
# ======================================================================
# COMPLETE UPDATED SCRIPT (PART 2 OF 3)
#   - QMD writer + render helper
#   - Main execution through bootstrap outputs
#   - Writes Bootstrap_EffDim_ByDay.csv ONCE (long format)
# ======================================================================

# -----------------------------
# QMD writer (MANDATORY) + render via rmarkdown::render (MANDATORY)
#   - Includes: metadata, runtime, header, dependencies, inventory
#   - Shows first 30 rows of each CSV output
# -----------------------------
write_qmd_report <- function(qmd_path,
  ...
  qmd <- c(
    ...
    "",
    "## Metadata",
    paste0("- **Run ID:** `", "{params$run_id}", "`"),
    paste0("- **Run timestamp:** `", "{params$run_timestamp}", "`"),
    paste0("- **Script name:** `", "{params$script_name}", "`"),
    paste0("- **Script path:** `", "{params$script_path}", "`"),
    paste0("- **Script full path:** `", "{params$script_full}", "`"),
    paste0("- **Output directory:** `", "{params$output_dir}", "`"),
    "",
    "## Runtime",
    "```{r}",
    "if (isTRUE(params$enable_runtime_tracking) && is.finite(params$runtime_seconds)) {",
    "  cat(sprintf(\"Total runtime: %.2f seconds\\n\", params$runtime_seconds))",
    "} else {",
    "  cat(\"Runtime tracking disabled (or runtime not recorded).\\n\")",
    "}",
    "```",
    "",
    ...
    "## Dependencies (packages + versions)",
    "```{r}",
    "pkgs <- c(\"data.table\",\"matrixStats\",\"changepoint\",\"ggplot2\",\"jsonlite\",\"knitr\",\"rmarkdown\")",
    "if (isTRUE(params$enable_plotly)) pkgs <- unique(c(pkgs, \"plotly\", \"htmlwidgets\"))",
    "pkgs <- pkgs[pkgs %in% rownames(installed.packages())]",
    "deps <- data.table::data.table(",
    "  package = pkgs,",
    "  version = vapply(pkgs, function(p) as.character(utils::packageVersion(p)), character(1))",
    ")",
    "knitr::kable(deps, format = \"html\")",
    "```",
    "",
    "## Generated files inventory",
    "```{r}",
    "inv_path <- file.path(params$output_dir, \"Project_Manifest_Files.csv\")",
    "if (file.exists(inv_path)) {",
    "  inv <- data.table::fread(inv_path)",
    "  knitr::kable(inv, format = \"html\")",
    "} else {",
    "  cat(\"Project_Manifest_Files.csv not found.\\n\")",
    "}",
    "```",
    "",
    ...
    "## Figures (static)",
    "```{r}",
    "img_files <- c(file.path(params$output_dir, \"Plots\", \"Metric_Trends_Annotated.png\"))",
    "img_files <- img_files[file.exists(img_files)]",
    "if (length(img_files)) knitr::include_graphics(img_files)",
    "```",
    "",
    "## Figures (interactive, plotly)",
    "```{r}",
    "if (isTRUE(params$enable_plotly)) {",
    "  ok <- requireNamespace(\"plotly\", quietly = TRUE)",
    "  if (!ok) {",
    "    cat(\"plotly not available; interactive plot skipped.\\n\")",
    "  } else {",
    "    ci_path <- file.path(params$output_dir, \"Table_1b_Daily_Metrics_BootstrapCI.csv\")",
    "    if (!file.exists(ci_path)) {",
    "      cat(\"Bootstrap CI table not found; interactive plot skipped.\\n\")",
    "    } else {",
    "      ci <- data.table::fread(ci_path)",
    "      ci[, Day := as.numeric(Day)]",
    "      p <- ggplot(ci, aes(x=Day, y=mid)) +",
    "        geom_pointrange(aes(ymin=lo, ymax=hi), linewidth=0.7) +",
    "        geom_line(linewidth=1) +",
    "        geom_point(size=2) +",
    "        facet_wrap(~Metric, scales=\"free_y\", ncol=1) +",
    "        theme_minimal(base_size=14) +",
    "        labs(title=\"Interactive: bootstrap median ± 95% CI\", x=\"Day\", y=NULL)",
    "      pw <- tryCatch(plotly::ggplotly(p), error=function(e) NULL)",
    "      if (is.null(pw)) {",
    "        cat(\"plotly::ggplotly failed; showing static ggplot instead.\\n\")",
    "        print(p)",
    "      } else {",
    "        pw",
    "      }",
    ...
    "  }",
    "} else {",
    "  cat(\"Interactive plots disabled.\\n\")",
    "}",
    "```",
    "",
    "## CSV outputs (first 30 rows each)",
    "```{r}",
    "csvs <- list.files(params$output_dir, pattern = \"\\\\.csv$\", full.names = TRUE)",
    "csvs <- csvs[!grepl(\"/Project_Manifest_Files\\\\.csv$\", csvs)]",
    "csvs <- sort(csvs)",
    "if (!length(csvs)) {",
    "  cat(\"No CSV outputs found.\\n\")",
    "} else {",
    "  for (f in csvs) {",
    "    cat(\"\\n\\n### \", basename(f), \"\\n\\n\", sep=\"\")",
    "    dt <- tryCatch(data.table::fread(f), error=function(e) NULL)",
    "    if (is.null(dt)) {",
    "      cat(\"Could not read this CSV.\\n\")",
    "      next",
    "    }",
    "    knitr::kable(utils::head(dt, 30), format = \"html\")",
    "  }",
    "}",
    "```",
    "",
    "## Interpretation guidance (Small-T)",
    "Emphasize robustness via (i) bootstrap support frequencies (Tables 2–5) and (ii) CI widths (Table 1b). Treat all “important day” calls as operational definitions; avoid mechanistic claims without orthogonal evidence.",
    "",
    "## Reproducibility",
    "```{r}",
    "sessionInfo()",
...
render_qmd_to_html <- function(qmd_path, output_html_path, params_list) {
  tryCatch({
    rmarkdown::render(
      input = qmd_path,
...
if (is.na(script_full)) {
  cat("NOTE: Script full-path detection failed in this execution mode; using fallback filename for labeling.\n")
}

# -----------------------------
# Prompts (plus Source prompt required for deterministic run_id)
# -----------------------------
B_TOTAL <- prompt_int("Replicate-stratified bootstraps (within-day SampleID resampling)", 200L)
CORES   <- prompt_int("Cores to use (this script uses a single loop; keep modest)", 1L) # retained for compatibility
...
logf(paste0("enable_plotly: ", isTRUE(enable_plotly)))
logf(paste0("enable_runtime_tracking: ", isTRUE(enable_runtime_tracking)))
...
# ------------------------------------------------------------
# NEW (SINGLE, CORRECT): Save Bootstrap_EffDim_ByDay.csv
#   Purpose: replicate-level effective dimensionality per day
#   Format: long (b, Day, eff_dim)
# ------------------------------------------------------------
logf("Writing Bootstrap_EffDim_ByDay.csv (replicate-level eff_dim by day; long format)...")

eff_mat <- boot_vals$eff_dim  # B_TOTAL x D
if (!is.matrix(eff_mat) || nrow(eff_mat) != B_TOTAL) {
  logf("WARNING: boot_vals$eff_dim missing or malformed; Bootstrap_EffDim_ByDay.csv not written.")
} else {
  if (is.null(colnames(eff_mat))) colnames(eff_mat) <- as.character(days_vec)
  
  Bootstrap_EffDim_ByDay <- data.table::as.data.table(eff_mat)
  Bootstrap_EffDim_ByDay[, b := seq_len(B_TOTAL)]
  Bootstrap_EffDim_ByDay <- data.table::melt(
    Bootstrap_EffDim_ByDay,
    id.vars = "b",
    variable.name = "Day",
    value.name = "eff_dim"
  )
  Bootstrap_EffDim_ByDay[, Day := as.integer(as.character(Day))]
  
  out_effdim <- file.path(output_dir, "Bootstrap_EffDim_ByDay.csv")
  data.table::fwrite(Bootstrap_EffDim_ByDay, out_effdim)
  register_output(out_effdim)
  logf("Wrote Bootstrap_EffDim_ByDay.csv")
}

# Bootstrap raw stat calls
...
# =========================
# END PART 2 OF 3
# Next: driver genes (canalization + per-pair transitions), plots, manifest, QMD render, console summary
# =========================
# ==============================
# PART 3 / 3
# (Paste this as the final third of the script)
# Starts immediately AFTER the bootstrap for-loop ends:
#   for (b in seq_len(B_TOTAL)) { ... }
# ==============================

# ------------------------------------------------------------
# Save Bootstrap_EffDim_ByDay.csv (one extra file)
# Purpose: expose bootstrap replicate-level effective dimensionality per day
# Output is LONG format: b, Day, eff_dim
# ------------------------------------------------------------
logf("Writing Bootstrap_EffDim_ByDay.csv (replicate-level eff_dim by day)...")
...
# ------------------------------------------------------------
# Save bootstrap raw stats (important-day calls)
# ------------------------------------------------------------
out_boot_raw <- file.path(output_dir, "Bootstrap_Raw_AllStats.csv")
data.table::fwrite(boot, out_boot_raw)
register_output(out_boot_raw)
...
# ------------------------------------------------------------
# Bootstrap CI table for each day × metric
# ------------------------------------------------------------
ci_dt <- data.table::rbindlist(lapply(names(boot_vals), function(met) {
  M <- boot_vals[[met]]
...
# ------------------------------------------------------------
# Bootstrap support tables (Tables 2–5)
# ------------------------------------------------------------
summ <- boot[is.finite(day), .N, by=.(stat, metric, day)]
summ[, support := N / B_TOTAL]
data.table::setorder(summ, stat, metric, -support)
...
# ------------------------------------------------------------
# Driver genes aligned to the RIGHT concepts
#   - Canalization drivers: big CV drop into canal_day (and stays low after)
#   - Transition drivers: per adjacent day-pair delta mean (balanced up/down)
# ------------------------------------------------------------
logf("Computing driver genes aligned to canalization/transition...")
...
# ------------------------------------------------------------
# Transition drivers for ALL adjacent day pairs
# Balanced: top positive AND top negative delta_mean per pair
# Writes: Table_7_DriverGenes_Transition_D{d1}_to_D{d2}.csv
# ------------------------------------------------------------
write_transition_driver_tables_all_pairs <- function(meanM, days_vec, output_dir,
                                                     top_pos = 200L, top_neg = 200L) {
  stopifnot(all(as.character(days_vec) %in% colnames(meanM)))
  
  for (i in 2:length(days_vec)) {
    d1 <- days_vec[i - 1]
    d2 <- days_vec[i]
    
    idx1 <- match(as.character(d1), colnames(meanM))
    idx2 <- match(as.character(d2), colnames(meanM))
    if (is.na(idx1) || is.na(idx2)) next
    
    delta_mean <- meanM[, idx2] - meanM[, idx1]
    ...
    pos <- DTt[delta_mean > 0][order(-delta_mean)][1:min(top_pos, .N)]
    neg <- DTt[delta_mean < 0][order(delta_mean)][1:min(top_neg, .N)]
    ...
    out_path <- file.path(output_dir,
                          sprintf("Table_7_DriverGenes_Transition_D%s_to_D%s.csv", d1, d2))
    data.table::fwrite(out, out_path)
    register_output(out_path)
  }
}

# Call ONCE (do not duplicate)
write_transition_driver_tables_all_pairs(meanM, days_vec, output_dir, top_pos = 200L, top_neg = 200L)
logf("Wrote Table_7_DriverGenes_Transition_D{d1}_to_D{d2}.csv for all adjacent day pairs")

# Canalization driver table (only if canal_day identified)
...
if (is.finite(canal_day)) {
  ...
  if (!is.na(idx) && idx > 1) {
    ...
    if (idx < ncol(cvM)) {
      stays <- is.finite(cvM[, idx]) &
        is.finite(cvM[, idx+1]) &
        (cvM[, idx+1] <= cvM[, idx] + 1e-9)
    }
    
    score <- ifelse(stays, drop_in, NA_real_)
    DTd <- data.table::data.table(Gene=rownames(cvM), canal_day=canal_day, drop_cv_into=as.numeric(score))
    ...
    logf(paste0("Wrote Table_6_DriverGenes_Canalization_D", canal_day, ".csv"))
  }
}

# Optional: single “transition day” driver table (legacy)
...
if (is.finite(infl_day)) {
  ...
  if (!is.na(idx) && idx > 1) {
    ...
    DTt <- data.table::data.table(Gene=rownames(meanM), infl_day=infl_day, delta_mean=as.numeric(delta_mean))
    DTt <- DTt[is.finite(delta_mean)]
    ...
    logf(paste0("Wrote legacy Table_7_DriverGenes_Transition_D", infl_day, ".csv"))
  }
}

# ------------------------------------------------------------
# Plots: bootstrap median ± 95% CI + annotated vertical lines
# ------------------------------------------------------------
...
p1 <- ggplot2::ggplot() +
  ggplot2::geom_pointrange(
    data = ci_dt_plot,
    ggplot2::aes(x = Day, y = mid, ymin = lo, ymax = hi),
    linewidth = 0.7
  ) +
  ggplot2::geom_line(
    data = ci_dt_plot,
    ggplot2::aes(x = Day, y = mid),
    linewidth = 1
  ) +
  ggplot2::geom_point(
    data = ci_dt_plot,
  ...
  ggplot2::geom_vline(
    data = anno,
    ggplot2::aes(xintercept = day, linetype = lab),
    linewidth = 0.8
  ) +
...
# ------------------------------------------------------------
# Manifest + QMD/HTML report (MANDATORY)
# ------------------------------------------------------------
deps_needed <- c("data.table","matrixStats","changepoint","ggplot2","jsonlite","knitr","rmarkdown")
if (isTRUE(enable_plotly)) deps_needed <- unique(c(deps_needed, "plotly", "htmlwidgets"))
deps_dt <- deps_table(deps_needed)

run_timestamp <- format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z")

end_time <- if (isTRUE(enable_runtime_tracking)) Sys.time() else NA
runtime_seconds <- if (isTRUE(enable_runtime_tracking)) {
  as.numeric(difftime(end_time, start_time, units = "secs"))
} else {
  NA_real_
}
if (isTRUE(enable_runtime_tracking) && is.finite(runtime_seconds)) runtime_seconds <- round(runtime_seconds, 2)
...
manifest <- list(
  ...
  parameters = list(
    ...
    enable_runtime_tracking = isTRUE(enable_runtime_tracking),
...
# Write QMD (uses function defined earlier in script)
write_qmd_report(
  qmd_path = qmd_path,
...
render_res <- render_qmd_to_html(qmd_path, html_path, params_list = params_list)
...
# Post-render inventory + manifest update
write_files_inventory(output_dir, manifest_files_csv)
...
if (is.na(script_full)) cat("  NOTE: script_full could not be detected; fallback filename was used.\n")

if (isTRUE(enable_runtime_tracking) && is.finite(runtime_seconds)) {
  cat("\nRuntime:\n")
  cat("  runtime_seconds: ", sprintf("%.2f", runtime_seconds), "\n", sep="")
}

cat("\nKey outputs:\n")
cat("  - Table_1_Daily_Metrics.csv\n")
cat("  - Table_1b_Daily_Metrics_BootstrapCI.csv\n")
cat("  - Bootstrap_EffDim_ByDay.csv   (replicate-level effective dimensionality)\n")
cat("  - Bootstrap_Raw_AllStats.csv\n")
...
cat("  - Table_6_DriverGenes_Canalization_D*.csv (if canal_day identified)\n")
cat("  - Table_7_DriverGenes_Transition_D*_to_D*.csv (all adjacent day pairs)\n")
